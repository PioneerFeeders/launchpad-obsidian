...
import { generateSearchFile,  getParentFolderFromPath,  getSearchFiles, getUnresolvedMarkdownFiles } from '../utils/getFilesUtils'
import { TextInputSuggester } from './suggester'
import { generateHotkeySuggestion } from '../utils/htmlUtils'
import { isValidExtension, type FileExtension, type FileType } from '../utils/getFileTypeUtils'
import { get } from 'svelte/store'
import HomeTabFileSuggestion from '../ui/components/homeTabFileSuggestion.svelte'
...
export default class LaunchpadFileSuggester extends TextInputSuggester<FuseResult<SearchFile>>{
    private files: SearchFile[]
    private fuzzySearch: FileFuzzySearch

    private view: View
    private plugin: Launchpad
    private searchBar: LaunchpadSearchBar

    private activeFilter: FileType | FileExtension  | null

    constructor(app: App, plugin: Launchpad, view: View, searchBar: LaunchpadSearchBar) {
        super(app, get(searchBar.searchBarEl), get(searchBar.suggestionContainerEl), {
                // @ts-ignore
                containerClass: `launchpad-suggestion-container ${Platform.isPhone ? 'is-phone' : ''}`,
                ...
                additionalModalInfo: plugin.settings.showShortcuts ? generateHotkeySuggestion([
                    ...
                    'launchpad-hotkey-suggestions') : undefined
    ...
    getSuggestions(input: string): FuseResult<SearchFile>[] {
        return this.fuzzySearch.rawSearch(input, this.plugin.settings.maxResults)
    }

    useSelectedItem(selectedItem: FuseResult<SearchFile>, newTab?: boolean): void {
    ...
    getDisplayElementProps(suggestion: FuseResult<SearchFile>): {nameToDisplay: string, filePath?: string}{
