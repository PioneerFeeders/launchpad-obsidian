...
import { getImageFiles, getMarkdownSearchFiles} from "../utils/getFilesUtils";
import type { FileType } from "../utils/getFileTypeUtils"

export const DEFAULT_FUSE_OPTIONS: IFuseOptions<any> = {
    includeScore : true,
    // includeMatches : true,
    // findAllMatches : true,
...
class fuzzySearch<T>{
    ...
    rawSearch(querry: string, limit?: number): FuseResult<T>[]{
...
export class MarkdownFileFuzzySearch extends fuzzySearch<MarkdownSearchFile>{
    constructor(fileList?: MarkdownSearchFile[], searchOptions?: IFuseOptions<MarkdownSearchFile>){
        const searchArray = fileList ?? getMarkdownSearchFiles()
        super(searchArray, searchOptions)
    }

    // Return the best match between the filename and the aliases
    getBestMatch(searchResultElement: FuseResult<MarkdownSearchFile>, querry: string): string{
        ...
        searchFile.aliases.forEach((alias: string) => searchArray.push(alias))
...
export class FileFuzzySearch extends fuzzySearch<SearchFile>{
    constructor(fileList: SearchFile[], searchOptions?: IFuseOptions<SearchFile>){
     ...
     */
    getBestMatch(searchResultElement: FuseResult<SearchFile>, querry: string): string{
        const searchFile = searchResultElement.item
...
export class ImageFileFuzzySearch extends fuzzySearch<TFile>{
    constructor(imageList?: TFile[], searchOptions?: IFuseOptions<TFile>){
